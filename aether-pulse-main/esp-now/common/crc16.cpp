...
// ═══════════════════════════════════════════════════════════════════
// Pre-computed table for polynomial 0x1021
// Significantly faster than bit-by-bit computation
...
uint16_t crc16_ccitt_byte(uint16_t crc, uint8_t byte) {
    ...
    return (crc << 8) ^ pgm_read_word(&crc16_table[index]);
}
...
bool crc16_verify(const uint8_t* data, size_t length) {
    ...
    uint16_t calculated = crc16_ccitt(data, length - 2);
    
    // Extract the appended CRC (little-endian)
